...
def get_data(data_name: str, *args, **kwargs) -> Any:
    """
    Decorator that fetches data from MessageBus and returns it directly.
    
    This decorator allows a method to simply return data from the MessageBus
    without having to call message_bus.get_data() explicitly.
    
    Example:
        @get_data('user_settings')
        def get_user_settings(self):
            # No implementation needed, decorator handles it
            pass
            
        # Can also pass args to the MessageBus data provider
        @get_data('user_info', user_id='default')
        def get_default_user(self):
            # Returns result of message_bus.get_data('user_info', user_id='default')
            pass
    
    Args:
        data_name: Name of the data to retrieve from MessageBus
        *args: Additional positional arguments to pass to the data provider
        **kwargs: Additional keyword arguments to pass to the data provider
        
    Returns:
        The data from the MessageBus
    """
    def decorator(func: Callable):
        @wraps(func)
        def wrapper(self, *func_args, **func_kwargs):
            try:
                # Merge kwargs from decorator and function call, with function call taking precedence
                combined_kwargs = {**kwargs, **func_kwargs}
                
                # Get the data from MessageBus
                result = self.message_bus.get_data(data_name, *args, *func_args, **combined_kwargs)
                return result
            except KeyError as e:
                # Handle the case where data provider doesn't exist
                error_msg = f"No data provider found for '{data_name}'"
                print(error_msg)
                # You could also raise the error again or return a default value
                return None
            except Exception as e:
                # Handle any other errors that might occur
                error_msg = f"Error getting data '{data_name}': {str(e)}"
                print(error_msg)
                return None
        return wrapper
    return decorator
