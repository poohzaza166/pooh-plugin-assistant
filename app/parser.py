...
class UnifiedCommandParser:
    ...
    def parse_input(self, text: str) -> Dict:
        ...
        if candidates:
            ...
            return {
                ...
                "parameters": params,
                "confidence": best_match[1] / (len(best_match[0].__dict__) - 2)  # Normalize score
            }
        
        # fallback to LLM
        functioncall_str = ""
        for cmd in self.commands.values():
            functioncall_str += str(cmd.to_claude_function_format())
        result = llm_functioncalling(querry=text, function_list=functioncall_str)
        print("--"*10)
        print(result)
        print("#"*10)
        if result != "":
            ...
            return{
                "command": func_name,
                "handler": self.commands[func_name].handler,  # Assuming the function name is the handler
                "parameters": params,
                "confidence": 1.0  # Placeholder for confidence calculation
            }
            # return commands

        # # Fallback to fuzzy matching if no exact matches
        # fuzzy_cmd = self._fuzzy_match(text)
        # if fuzzy_cmd:
        #     params = self._extract_parameters(self.nlp(text), fuzzy_cmd)
            
        #     # Check if needs context
        #     if fuzzy_cmd.required_entities and not all(params.get(e) 
        #         for e in fuzzy_cmd.required_entities):
        #         self.context = Context()
        #         self.context.active_command = fuzzy_cmd.name
        #         self.context.slots = params
                
        #         return {
        #             "command": fuzzy_cmd.name,
        #             "confidence": 0.75,  # Default confidence for fuzzy matches
        #             "context_prompt": self._generate_slot_prompt(fuzzy_cmd),
        #             "parameters": params
        #         }
            
        #     return {
        #         "command": fuzzy_cmd.name,
        #         "handler": fuzzy_cmd.handler,
        #         "parameters": params,
        #         "confidence": 0.75,  # Fixed initial value since score isn't defined here
        #         "fuzzy_match": True
        #     }
    ...
    def execute_command(self, text: str):
        ...
        if "error" in result:
            return result
        print(result)
        output = execute_with_matched_params(result["handler"], result["parameters"])
        return output
...
def execute_with_matched_params(handler: Callable, parameters: Dict[str, Any]) -> Any:
    ...
    if extra_params:
        ...
        print(f"[INFO] Extra parameters not used by {handler.__name__}: {extra_params}")
